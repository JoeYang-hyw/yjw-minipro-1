...
Page({
  ...
  getCityInfo: function (latitude, longitude) {
    var that=this
    wx.request({
      url: "https://api.map.baidu.com/reverse_geocoding/v3//?ak=cFTUTwgTdt6Gv3hlaVzt3zOKytnKzcAz&output=json&coordtype=wgs84ll&location=" + latitude + ',' + longitude,
      success: function (res) {
        //console.log(res)
        var currentCity = res.data.result.addressComponent.district
        var curProvince = res.data.result.addressComponent.city
        that.setData({
          city:currentCity,
          province:curProvince
        })
        that.getLocalId(that.data.city,that.data.province)  
      },
      fail: function () {
        console.log("定位失败")
      }
  ...
  getLocalId:function(district,province){
    var that=this
    wx.request({
      url: "https://geoapi.qweather.com/v2/city/lookup?location=" + district + "&adm="+ province + "&key=1f8b48aec85645a8a8478c96bd641292",
      success:function(res){
        console.log(res)
        that.setData({
        ...
        app.getWeatherInfo(that.data.cityid,function(data){
          that.setData({
            weatherInfo:data.daily
          })
        })
        app.getLifestyleInfo(that.data.cityid, function (data) {
          that.setData({
            lifestyle: data.daily
          })
        })
        app.getCurWeatherInfo(that.data.cityid,function(data){
          that.setData({
            curWeather: data.now
          })
        })    
  ...
  confirm:function(){
    ...
    var province=this.data.cities[this.data.values[1]]
    this.setData({
      province:province,
      city:district,
      selectCityView:false
    })
    that.getLocalId(district,province)
  },
